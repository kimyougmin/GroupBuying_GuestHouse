...
구조적으로 10개을 데이터를 하나의 요청으로 처리하는 것이 아니라 10개의 데이터를 10개의 요청으로 분리하고 각 요청을 컴포넌트 하나의 레벨로 분리해야 합니다.
메인 페이지에서 랜러딩되는 카드의 개수는 화면 크기에 영향을 받아야하기 때문에 랜더링이 완료되었을 때의 혹은 사이즈 변경이 끝나는 시점에 데이터를 조회하려 하였으나 이는 랜더링이 완료된 시점에서 다시 랜더링을 위한 데이터를 조회하는 것이기 때문에 성능 이슈는 있을 것입니다. 
